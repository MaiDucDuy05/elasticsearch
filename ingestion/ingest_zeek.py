...
from validator import validate_zeek
from bulk_indexer import get_client, bulk_index
from config import ZEEK_INDEX

logging.basicConfig(level=logging.INFO,
                    format="%(asctime)s %(levelname)s %(message)s")
logger = logging.getLogger(__name__)

def load_zeek(filepath: str) -> list[dict]:
...
def run(filepath: str):
    client = get_client()
    docs   = load_zeek(filepath)

    if not docs:
        logger.error("No valid docs to index.")
        return

    success, errors = bulk_index(docs, ZEEK_INDEX, client)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python ingest_zeek.py <zeek_ndjson_file>")
        sys.exit(1)
    run(sys.argv[1])